...
/**
 * Handles all request concerning the search for an ad and the results. It uses
 * the AdService Class.
 *
 */
@Controller
public class SearchController {
	...
	public ModelAndView results(@Valid SearchForm searchForm, BindingResult result) {
		mainlog.log.warning("SearchController method results received a request with the following searchForm: "
				+ searchForm.toString());
		if (!result.hasErrors()) {
			ModelAndView model = new ModelAndView("results");
			model.addObject("results", adService.searchAndFilter(searchForm));
			mainlog.log.warning("SearchController method results caused an error with the following searchForm: "
					+ searchForm.toString());
			return model;
		} else {
			// go back
			mainlog.log.warning("SearchController method results processed request with the following searchForm: "
					+ searchForm.toString());
			return searchAd();
		}
	}
